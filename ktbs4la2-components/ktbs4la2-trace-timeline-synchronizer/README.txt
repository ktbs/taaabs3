DESCRIPTION :
-------------
This component allows to synchronize several instances of <ktbs4la2-trace-timeline> from it's subtree.
The synchronizable elements are the stylesheets, the timelines' cursors (= the vertical red bar over the timeline) and the timelines' views (zoom and position of the displayed portion of the timeline)

It provides one custom element : <ktbs4la2-trace-timeline-synchronizer> which is described below
...
---------------------
    Attributes :
    ------------
        - sync-stylesheets (optional)
            Description : whether or not we should synchronize the child timelines stylesheets
