...
export class Trace extends Resource {
	 ...
	 */
	_getPutData() {
		let putData = super._getPutData();

		/*if(putData.hasObselList)
			delete putData.hasObselList;

		if(putData.hasTraceStatistics)
			delete putData.hasTraceStatistics;

		if(putData.isSourceOf)
			delete putData.isSourceOf;*/
	...
	get method() {
		if(!this._method) {
			...
			//try {
				...
				this._method = ResourceProxy.get_resource(Method, method_uri);
			/*}
			catch(error) {
				this._method = Method.getBuiltinMethod(method_id);
			}*/
		}
		...
		if(!this._source_traces) {
			...
			if(
				...
				for(let i = 0; i < this._JSONData.hasSource.length; i++) {
					...
					const trace = ResourceProxy.get_resource(Trace, trace_uri);
